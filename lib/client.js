...
class Client extends EventEmitter {
  /**
   * constructor
   * @param  {String} uri - The influxdb connection uri, eg: 'http://user:pass@localhost:port,anotherhost:port,yetanother:port'
   * @param  {String} db - The influxdb database name, eg: 'mydatabase'
  ...
  constructor(uri, db, options) {
    ...
    if (!result || !result[1] || !result[3]) {
      const message = 'Connect URI is wrong, eg: http://user:pass@localhost:port,anotherhost:port,yetanother:port';
